 ...
 * <p>
 * The description always begins with the keyword <tt>IntegerOPDF</tt>. The next
 * (resp. last) symbol is an opening (resp. closing) bracket. Between the
 * backets is a list of numbers separated by a space. The <tt>i</tt>-th number
 * is the probability of <tt>i-1</tt>.
 * <p>
 * For example, reading <tt>IntegerOPDF [ .2 .3 .5 ]</tt> returns a distribution
 * equivalent to <code>new OpdfInteger(new double[] { .2 .3 .5 })</code>.
 */
public class OpdfIntegerReader
     ...
     *
     * @param nbEntries The number of entries that should be found (<i>i.e.</i>
     * a {@link FileFormatException} is triggered if the read <code>opdf</code>
     * is not such as <code>opdf.nbEntries() == nbEntries</code> ).
     */
